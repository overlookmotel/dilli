...
// Imports
const {isObject, isBoolean, promiseFirstResolve} = require('./utils'),
...
module.exports = {
	...
	_conformMessageOptions(options) {
		if (isObject(options) && isBoolean(options.persist) && isBoolean(options.mustDeliver)) {
			return options;
		}

		options = {...options};
		options.persist = options.persist == null ? true : !!options.persist;
		options.mustDeliver = options.mustDeliver == null ? options.persist : !!options.mustDeliver;
		return options;
	},
	...
	async _messageAwait(deliveryPromise, persistPromise) {
		...
		if (persistPromise || deliveryPromise) {
			if (!deliveryPromise) {
				...
				delivered = resolvedIndex === 0; // TODO Need to look at return value of deliveryPromise
				persisted = resolvedIndex === 1;
			}
		}
