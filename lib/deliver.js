...
// Modules
const {defer, promiseWait} = require('promise-methods');
 ...
 *   - If currently not connected to server and message marked `persist: false`, it is dropped
...
module.exports = {
	...
	_deliver(messageId, message, options) {
		...
		if (this.connectState !== STARTED && !options.persist) return null;
	...
	_flushQueue() {
		...
		for (let i = this._numMessagesSending; i < queue.length; i++) {
			...
			const promise = this.deliver(messageId, message, options);
			if (!promise) {
				// Delivery says it can't handle more
				// TODO Drop non-persisted messages?
				this._paused = true;
				break;
			}
	...
	_handleDeliveryResult(item, delivered) {
		...
		if (!delivered) {
			...
			if (!item.options.persist) {
				const index = queue.findIndexOf(item);
	...
	async deliver(messageId, message, options) {
		// TODO Write this!
		this.log.info('Delivering message', {messageId, message, options});
