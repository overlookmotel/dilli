...
// Modules
const {allAwait, tapCatch} = require('promise-methods');
...
module.exports = {
	...
	async _stop() {
		...
		const promises = [];
		forIn(this.workers, (worker, name) => {
			...
			promises.push(
				tapCatch(
					...
					err => log.error('Error stopping worker', {worker: name, err})
				)
			);
		});
		...
		try {
			...
			await promiseTryWrap(() => this.journal.close());
			log.info('Closed journal');
		} catch (err) {
			this._journalErr = err;
			if (!closeErr) closeErr = err;
		...
		try {
			...
			await promiseTryWrap(() => this.connection.close());
		...
		// If error in stopping, log and throw error
		if (closeErr) {
			this._stopStatus = ERRORED;
			this._stopPromise = null;
			log.error('Error stopping', {err: closeErr});
			throw closeErr;
		}

		// Stopping completed cleanly
