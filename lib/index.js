...
class Dilli {
	...
	attachWorker(worker) {
		...
		let name = worker.nameFull;
		if (name != null) {
			if (!isFullString(name)) throw new Error('worker.nameFull must be a string if defined');
		} else {
			name = worker.name;
			if (!isFullString(name)) throw new Error('worker.name or worker.nameFull must be a string');
			name = uncapitalize(name);
			worker.nameFull = name;
		}

		const {version} = worker;
